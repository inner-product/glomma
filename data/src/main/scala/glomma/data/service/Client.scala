...
final case class Client(scenario: Scenario, configuration: Configuration) {
  ...
  def events(): LazyList[Event] = {
    val badEvents: LazyList[Event] =
      LazyList.continually(badEvent.generate.sample())

    val goodEvents: LazyList[Event] =
      LazyList.unfold((List.empty[Event], scenario.sessions)) {
    ...
    val evts = LazyList.unfold((badEvents, goodEvents)) { case (bad, good) =>
      if (scala.util.Random.nextDouble() < 0.2)
        Some((bad.head, (bad.tail, good)))
      else if (goodEvents.isEmpty) None
  ...
  def run(): IO[ExitCode] = {
    BlazeClientBuilder[IO](global).resource.use { client =>
      ...
      val postBooks: IO[Unit] =
        client.expect[Unit](
          POST(
            scenario.books.map(b => glomma.event.Book(b.name, b.price)).asJson,
            configuration.bookUrl
          )
        )
      ...
      val postEvents: IO[Unit] = {
        def loop(events: LazyList[Event]): IO[Unit] =
          if (events.isEmpty) IO(())
            ...
            client
              .expect[Unit](POST(events.head.asJson, configuration.eventUrl))
              .flatMap(_ => loop(events.tail))
